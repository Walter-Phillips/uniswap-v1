...
contract Exchange is ERC20 {
    ...
    function addLiquidity(uint256 _tokenAmount) public payable returns(uint256){
        if(getReserve()==0){
            ...
            require(_tokenAmount >= tokenAmount,"insuffieient funds");
    ...
    function getPrice(uint256 inputReserves, uint256 outputReserves) public pure returns (uint256){
        require(inputReserves>0 && outputReserves>0, "invalid reserves");

        return (1000 * inputReserves)/outputReserves;
    }

    function getAmount(uint256 inputAmount, uint256 inputReserve, uint256 outputReserve) private pure returns (uint256) {
        require(inputReserve > 0 && outputReserve > 0, "invalid reserves");

        uint256 inputAmountWithFees = inputAmount * 99.7;
    ...
    function tokenToEthSwap(uint256 _minEth, uint256 _tokensSold) public payable {
        uint256 tokenReserve = getReserve();
